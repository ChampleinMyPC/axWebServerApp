...
static char *del  = {"llllllllllllllllllllllllll"};
...
{
  syslog(LOG_WARNING, "Failed to get Auth0 token: 111111111");

  CURL *curl = curl_easy_init();
  if (!curl)
    return FALSE;
  syslog(LOG_WARNING, "Failed to get Auth0 token: 222222222");
...
static int try_ship_file_to_node(const char *filepath, const char *api_key)
{
  ...
  syslog(LOG_WARNING, "appel de http_post_json  : %s", del);
  gboolean ok = http_post_json(api_key, contents, &code);
  syslog(LOG_WARNING, "fin de l'appel http_post_json  : %s", del);
  syslog(LOG_WARNING, "Failed to get Auth0 token: %s", del);
...
static int SendHandler(struct mg_connection *conn, void *cb)
{
  ...
  if (!body)
  {
    mg_send_http_error(conn, 500, "OOM");
    return 500;
  }
  ...
  while (got < (size_t)need)
  {
    ...
    {
      free(body);
      mg_send_http_error(conn, 400, "Body read failed");
      return 400;
    }
    got += (size_t)r;
  }
  ...
  while ((p = strstr(p, "\"/")))
  {
    ...
    {
      if (so)
        so += (size_t)snprintf(shipped + so, sizeof(shipped) - so, ",");
...
static gboolean http_download(const char *url, const char *out_path, char err[256])
{
  ...
  if (!c)
  {
    g_snprintf(err, 256, "curl init fail");
    return FALSE;
  }
  FILE *f = fopen(out_path, "wb");
  if (!f)
  {
    g_snprintf(err, 256, "open %s fail", out_path);
    curl_easy_cleanup(c);
    return FALSE;
  }
  ...
  if (rc != CURLE_OK)
  {
    g_snprintf(err, 256, "download rc=%d", rc);
    unlink(out_path);
    return FALSE;
  }
  return TRUE;
                                     ...
                                     char *out, size_t outsz, char err[256])
{
  ...
  if (!p)
  {
    snprintf(err, 256, "app '%s' not found", app_id);
    free(m.p);
    return FALSE;
  }
  ...
  if (!v)
  {
    snprintf(err, 256, "Version attribute missing");
    free(m.p);
    return FALSE;
  }
  v += strlen("Version=\"");
                                 ...
                                 char *out, size_t outsz, char err[256])
{
  for (int i = 0; i < 10; ++i)
  { // 10 tentatives max
    if (axis_app_version_xml(user, pass, app_id, out, outsz, err))
      return TRUE;
...
static void make_upload_filename(const char *eap_url, char *fname, size_t sz)
{
  ...
  if (dot && strcmp(dot, ".eap") == 0)
  {
    ...
    if (stem_len > sz - 32)
      stem_len = sz - 32; // garde une marge
                                      ...
                                      char err[256])
{
  ...
  if (!ok)
  {
    g_snprintf(err, 256, "upload ok but no package name in response");
    return FALSE;
  }
  return TRUE;
}
                                  ...
                                  char err[256])
{
  ...
  char fname[128];
  make_upload_filename(eap_url, fname, sizeof fname);
  char tmp[256];
  snprintf(tmp, sizeof tmp, "/tmp/acap_updates/%s", fname);
  syslog(LOG_INFO, "DOWNLOAD OF RELEASE");
  if (!http_download(eap_url, tmp, err))
    return FALSE;
  ...
  char ver[64] = {0};
  if (!wait_for_version(cam_user, cam_pass, APP_ID, ver, sizeof ver, err))
    return FALSE;
...
{
  int d = 0;
  if (!file_mtime_days_ago(stamp_path, &d))
    return TRUE; // pas de fichier => check
  return d >= days;
}
gboolean touch_update_stamp(const char *stamp_path, char err[128])
{
  FILE *f = fopen(stamp_path, "ab");
  if (!f)
  {
    g_snprintf(err, 128, "touch fail");
    return FALSE;
  }
  fclose(f);
                                    ...
                                    char err[256])
{
  ...
  if (!c)
  {
    snprintf(err, 256, "curl_easy_init");
    return NULL;
  }
  ...
  {
    char auth[320];
    snprintf(auth, sizeof auth, "Authorization: Bearer %s", oauth_token);
  ...
  if (rc != CURLE_OK)
  {
    snprintf(err, 256, "GitHub GET failed: %s", curl_easy_strerror(rc));
    free(m.p);
    return NULL;
  }
  if (http != 200)
  {
    snprintf(err, 256, "GitHub HTTP %ld", http);
    free(m.p);
    return NULL;
  }
  ...
  if (!assets)
  {
    snprintf(err, 256, "No assets[] in latest release");
    free(m.p);
    return NULL;
  }
  const char *arr = strchr(assets, '[');
  if (!arr)
  {
    snprintf(err, 256, "assets not an array");
    free(m.p);
    return NULL;
  }
  ...
  while ((cur = strstr(cur, "\"name\"")) != NULL)
  {
    ...
    if (!name)
    {
      cur += 6;
      continue;
    } // avance pour éviter boucle infinie
    int is_eap = 0;
